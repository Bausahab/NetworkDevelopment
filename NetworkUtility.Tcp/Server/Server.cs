...
namespace NetworkUtility.Tcp.Server
{
    public class Server
    {
      ...
      public List<ClientReference> _clients = new List<ClientReference>();
        //gather information to show in list
        List<string> ConectedClients = new List<string>();
        ...
        public delVoidCurrentSocket ClientConnected { get; set; }
        public delVoidCurrentSocket ClientDisconnected { get; set; }
        //public delVoidClientReference ClientDtl { get; set; }
        ...
        private void DataAccept(Socket sock)
        {
            ...
            try
            {
                ...
                _context.Post(s =>
                {
                    ClientConnected?.Invoke(sock);
                    //ClientDtl?.Invoke(sock);

                }, null);
                //ClientDtl?.Invoke(client);
                //
            }
            catch (Exception ex)
            {
                //dont trust user code
                Console.WriteLine("Calling server ClientConnected event handler threw an exception"+ex.Message);
            }
        ...
        private void DataReceived(ClientReference client)
        {
            ...
            if (client.BytesReceived == 0)
            {
                try
                {
                    ...
                    _context.Post(s =>
                    {
                        ClientDisconnected?.Invoke(client.sock);
                    }, null);
        ...
        //public delegate void delVoidClientReference(Socket sok);
        private delegate void delVoidSock(Socket s);
        public delegate void delVoidVoid();
        public delegate void delVoidCurrentSocket(Socket CC);
